 ...
 */
import { Items, ItemStack, MinecraftBlockTypes, Player, world } from "@minecraft/server";
...
world.events.beforeItemUseOn.subscribe(res => {
    ...
    const rawtext = text || JSON.parse(signComp.getRawText()?.rawtext?.[1].score.name);
    if (rawtext?.enabled) {
        res.cancel = true;
        switch (rawtext.type) {
            case '0':
                sell(player, rawtext.itemId, rawtext.amount, rawtext.price);
                break;
            case '1':
                buy(player, rawtext.itemId, rawtext.amount, rawtext.price);
                break;
        }
        return;
    }
...
const getScore = (objectiveId, identity) => {
...
export function sell(player, itemId, quantity, price) {
...
export function buy(player, itemId, quantity, price) {
    quantity = Number(quantity), price = Number(price);
    const itemType = Items.get(itemId);
    if (!itemType || isNaN(price) || isNaN(quantity))
        return;
